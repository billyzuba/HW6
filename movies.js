...
window.addEventListener('DOMContentLoaded', async function (event) {
  ...
  let movieData = await fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${apiKey}&language=en-US`)
  let movieResults = await movieData.json()
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    let moviePosterPath = movies[i].poster_path
  ...
  let moviesWatched = await db.collection('movies').get()
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    button.addEventListener('click', async function (event) {
      event.preventDefault()
      let movieIdText = movies[i].id
      let movieTitleText = movies[i].original_title

      if (movies.length > 0) {
        let movieRef = await db.collection('movies').add({
          movieId: movieIdText
